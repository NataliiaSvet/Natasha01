...
for i in range(num_bars):
    ...
    ax.text(x, y, heights.iloc[i] + 0.5, f'{int(heights.iloc[i])}%', ha='center', va='bottom', fontsize=1)

# Настройки осей
ax.set_xticks(x_positions)
ax.set_xticklabels(directions_sorted, rotation=45, ha='right', fontsize=1)
...
plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)  # Регулировка отступов
