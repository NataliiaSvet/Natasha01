...
# --- Основной график и панель выбора периода для него ---

# Заголовок для главного графика
st.title("График расходов и доходов от миграции")

# Создание контейнера для боковой панели только для графика
with st.sidebar:
    st.header("Выберите период времени для графика")
    periods = df['Период времени'].unique()  # Получаем уникальные периоды времени из данных
    selected_period = st.selectbox("Период времени", periods)
...
# --- Другие графики и панель выбора периода для них ---

# Если у вас есть другие графики, они могут быть добавлены ниже без влияния на текущий график
# Например, создание еще одного графика без изменения фильтрации:

st.header("Другие графики")

# Можно создать другие графики, не влияя на первый график
# ...
