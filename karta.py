...
if 'Широта' not in df.columns or 'Долгота' not in df.columns or 'Регион' not in df.columns or 'Количество беженцев' not in df.columns:
    st.error("Пожалуйста, убедитесь, что ваш файл Excel содержит все необходимые колонки: Широта, Долгота, Регион, Количество беженцев.")
else:
    # Создание карты
